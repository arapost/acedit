...
import GSN1.diagram.edit.commands.ContractCreateCommand;
...
public class CaseItemSemanticEditPolicy extends GSN1BaseItemSemanticEditPolicy {
	...
	protected Command getCreateCommand(CreateElementRequest req) {
		...
		if (GSN1ElementTypes.Solution_2003 == req.getElementType()) {
			return getGEFWrapper(new SolutionCreateCommand(req));
		}
		if (GSN1ElementTypes.Context_2004 == req.getElementType()) {
			return getGEFWrapper(new ContextCreateCommand(req));
		}
		if (GSN1ElementTypes.Justification_2005 == req.getElementType()) {
			return getGEFWrapper(new JustificationCreateCommand(req));
		}
		if (GSN1ElementTypes.Assumption_2006 == req.getElementType()) {
			return getGEFWrapper(new AssumptionCreateCommand(req));
		}
		if (GSN1ElementTypes.Contract_2007 == req.getElementType()) {
			return getGEFWrapper(new ContractCreateCommand(req));
		}
		if (GSN1ElementTypes.ArgumentModule_2008 == req.getElementType()) {
